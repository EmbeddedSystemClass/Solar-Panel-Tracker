...
#include "pwm.h"
//#include "commands.h"   for LCD
#include <stdio.h>           // printf
#define ADC_MASK 0xFFF // masks 12-bits
#define ADC_MAX_RANGE (double)(1 << 12) // maximum value readable by ADC
#define SIZE 96
char dataMem[SIZE];
#define degree_zero 0.8
...
//#define ADC_MASK 0xFFF // masks 12-bits
//#define ADC_MAX_RANGE (double)(1 << 12) // maximum value readable by ADC
bool Photo_Sensor::init()
{
    //printf("Photo_init()\n");
  // LPC_PINCON->PINSEL3 &= ~(3 << 28);
   // LPC_PINCON->PINSEL3 |= (3 << 28); // Initialize P1.30 as AD0.4
    adc0_initialize(4);// change

    return true;
}
unsigned short Photo_Sensor::getRawValue()
{
    //printf("Photo_getRawValue()\n");
    return adc0_getReading(mAdcChannelOfSensor);
}
unsigned short Photo_Sensor::getServoValue()
{
    //printf("Photo_getRawValue()\n");
    const unsigned int rawValue = ADC_MASK & adc0_getReading(mAdcChannelOfSensor);


    return rawValue;
}
unsigned short Photo_Sensor::getPercentValue()
{
   //printf("Photo_getPercentValue()\n");
    const unsigned int rawValue = ADC_MASK & adc0_getReading(mAdcChannelOfSensor);
    return (rawValue * 100) / ADC_MAX_RANGE;
}
//================================================================
bool Servo_Motor::init()
{
    printf("servo_motor_init()\n");
    LPC_PINCON->PINSEL3 |= (2 << 14); // Set pin P1.23 as PWM1.4
    //LPC_PINCON->PINSEL3 |= (2 << 16); // Set pin P1.23 as PWM1.5
    initPWM(200);//96*10000, 96*2

    return true;
}

void Servo_Motor::change_direction(float milliseconds){
    pwmEn(1);

    /**
     * Objective is allow user to give value in milliseconds
     * and calculate the corresponding MR value
     *
     * 1.1
     */
    if (milliseconds > 20) {
        //LOG_ERROR("Invalid value given");
        return;
    }

    float max_value = LPC_PWM1->MR0;
    float freq_ms = 20.0;
    float target_value = (milliseconds / freq_ms) * max_value;
    pwmUpdateMR(4, target_value);
...
//        if(duty_cycle>13)
//            duty_cycle = 13;
//      //  printf("Duty Cycle: %i pulse Width: %i\n",duty_cycle, pwmReadMR(0));
//    pwmUpdateMR(4, (pwmReadMR(0) * duty_cycle) / 100);
//   // printf("pulse Width: %i\n", pwmReadMR(4));
}

void Servo_Motor::set_degree(float degree){
    pwmEn(1);

    /**
     * Objective is allow user to give value degrees
     * and calculate the corresponding MR value
     *
     * 1.1
     */
    ...
    float max_value = LPC_PWM1->MR0;
    float freq_ms = 20.0;
    degree=(degree/180)*1.8 +.7;
    float target_value = (degree / freq_ms) * max_value;
