...
/**
 * Prints memory information to your buffer.
 ...
 *      }
 */
extern xSemaphoreHandle critialRegionSemaphore;
#define CRITICAL_REGION() \
        for(char rtosStarted = isFreeRtosRunning(),         \
                    lock = rtosStarted ? xSemaphoreTake(critialRegionSemaphore, portMAX_DELAY) : 1; \
                    0 != lock; \
                    lock = rtosStarted ? !xSemaphoreGive(critialRegionSemaphore) : 0)
