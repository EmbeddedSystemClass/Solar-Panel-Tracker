 ...
 */
#define STR_ON_STACK(name, size)     char __##name##buffer[size]; str name((__##name##buffer), sizeof(__##name##buffer))
...
class str
{
    ...
    private:
        ...
        static const int mInitSize = 16;

        /// init() is called by constructors to initialize the string
        void init(int initialLength=mInitSize)
        {
            mStackMem = false;
            mCapacity = 0;
            mpStr = 0;
